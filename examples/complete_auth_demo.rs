use anyhow::Result;
use diap_rs_sdk::{AgentAuthManager, AuthResult};
use std::env;
...
async fn main() -> Result<()> {
    ...
    // 创建Alice
    let (alice_info, alice_keypair, alice_peer_id) =
        auth_manager.create_agent("Alice", Some("alice@example.com"))?;

    // 注册Alice身份
    println!("\n📝 注册Alice身份...");
    let alice_registration = auth_manager
        .register_agent(&alice_info, &alice_keypair, &alice_peer_id)
        .await?;
    println!("✅ Alice身份注册成功");
    ...
    // 创建Bob
    let (bob_info, bob_keypair, bob_peer_id) =
        auth_manager.create_agent("Bob", Some("bob@example.com"))?;

    // 注册Bob身份
    println!("\n📝 注册Bob身份...");
    let bob_registration = auth_manager
        .register_agent(&bob_info, &bob_keypair, &bob_peer_id)
        .await?;
    println!("✅ Bob身份注册成功");
    ...
    let (alice_proof, bob_verify_alice, bob_proof, alice_verify_bob) = auth_manager
        .mutual_authentication(
            &alice_info,
            &alice_keypair,
            &alice_peer_id,
            &alice_registration.cid,
            &bob_info,
            &bob_keypair,
            &bob_peer_id,
            &bob_registration.cid,
        )
        .await?;
    ...
    // 批量认证测试
    let batch_result = auth_manager
        .batch_authentication_test(
            &alice_info,
            &alice_keypair,
            &alice_peer_id,
            &alice_registration.cid,
    ...
    if proof_count > 0 {
        println!(
            "   证明生成平均时间: {}ms",
            total_proof_time / proof_count as u64
        );
    }
    if verify_count > 0 {
        println!(
            "   身份验证平均时间: {}ms",
            total_verify_time / verify_count as u64
        );
    }
