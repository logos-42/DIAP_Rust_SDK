...
use anyhow::Result;
use diap_rs_sdk::{AgentInfo, IpfsBidirectionalVerificationManager, KeyPair};
use std::env;
...
async fn main() -> Result<()> {
    ...
    println!(
        "验证时间戳: {}",
        bidirectional_result.verification_timestamp
    );
    ...
    // 注册新智能体
    println!("\n📤 注册Charlie到IPFS...");
    let _charlie_cid = verification_manager
        .register_agent(&charlie_info, &charlie_keypair)
        .await?;

    println!("\n📤 注册David到IPFS...");
    let _david_cid = verification_manager
        .register_agent(&david_info, &david_keypair)
        .await?;
    ...
    let batch_results = verification_manager
        .batch_bidirectional_verification(agent_pairs, resource_cid)
        .await?;
    ...
    println!(
        "   成功率: {:.1}%",
        (success_count as f64 / batch_results.len() as f64) * 100.0
    );
    ...
    println!(
        "整体成功率: {:.1}%",
        (total_successful_pairs as f64 / total_pairs as f64) * 100.0
    );
    ...
    if verification_count > 0 {
        ...
        println!(
            "验证吞吐量: {:.2} 对/秒",
            (verification_count as f64 * 1000.0) / total_verification_time as f64
        );
    }
